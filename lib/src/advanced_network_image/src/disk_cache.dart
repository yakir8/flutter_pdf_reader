...
import 'package:flutter_pdf_reader/src/advanced_network_image/src/utils.dart' show crc32, uid;
...
class DiskCache {
  ...
  int get _currentSizeBytes {
    int size = 0;
    if (_metadata != null) {
      _metadata!.values.forEach((item) => size += int.parse(item['size'].toString()));
    }
    return size;
  }
